...
from freedm.utils.rpyc import AsyncIOServer
...
class GenericDaemon(AsyncIOServer):
    '''
    ...
    def _getRpcConnections(self):
        ...
        if isinstance(self, ThreadPoolServer):
            ...
            return connections
        elif isinstance(self, (ThreadedServer, AsyncIOServer)):
    ...
    def _accept_method(self, *args):
        ...
        if self.state in ('stopping', 'idle', 'crashed'):
            if isinstance(self, (ThreadedServer, ThreadPoolServer)):
                # Close the socket immediately because daemon is halting or already halted.
                sock = args[0]
                sock.close()
            elif isinstance(self, AsyncIOServer):
                # Close the writer immediately because daemon is halting or already halted.
                writer = args[1]
                writer.close()
        else:
            # Call the overriden function
            super()._accept_method(*args)
